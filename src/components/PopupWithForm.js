  ...
  return (
    <div className={`popup popup_type_${props.name} popup_transparent_medium ${props.isOpen && popupConfig.popupOpenedClass}`}>
      <div className="page__container">
        ...
        <form name={props.name} className="page__form" noValidate onSubmit={props.onSubmit}>
          <h2 className={`page__form-title ${props.onlyTitle ? 'page__form-title_onlytitle' : ''}`}>{props.formTitle}</h2>
          {props.children}
          <button className="page__form-submit-btn" type="submit">{props.isSaving ? popupConfig.popupWithFormSavingText : props.submitButtonText}</button>
        </form>
      </div>
    </div>
  );
