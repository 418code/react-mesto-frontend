...
export default function EditProfilePopup(props) {
  ...
  const {
    ...
    isValid,
    resetForm,
  } = useFormWithValidation(true, currentUser);
  const intl = useIntl();
  ...
  return (
    <PopupWithForm
      name={popupConfig.profileEditPopupAndFormName}
      ...
      isSubmitDisabled={disabledButtonCondition}
    >
      <input
        id="popup__profile-name-input"
        type="text"
        name="name"
        className="page__form-text"
        placeholder={intl.formatMessage({
          id: "Jack",
          defaultMessage: "Жак-Ив Кусто",
        })}
        minLength="2"
        maxLength="40"
        required="required"
      ...
      />
      <span
        className={`page__form-text-error ${
      ...
      <input
        id="popup__profile-description-input"
        type="text"
        name="about"
        className="page__form-text"
        placeholder={intl.formatMessage({
          id: "ocean_explorer",
          defaultMessage: "Исследователь океана",
        })}
        minLength="2"
        maxLength="200"
        required="required"
      ...
      <select
        ...
        name="locale"
        value={values["locale"] || currentUser.locale}
        onChange={handleChange}
      >
