...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState(
    JSON.parse(localStorage.getItem("currentUser")) || blankCurrentUser
  );
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      api
        ...
        .catch((err) => {
          console.log(err);
          if (err.indexOf("404") === -1) showFailPopup();
        });
    }
  }, [isLoggedIn, showFailPopup]);

  useEffect(() => {
    if (infoTooltipContent.message !== "blank") setIsInfoTooltipOpen(true);
  }, [infoTooltipContent]);

  useEffect(() => {
    if (
      imagePopupContent.card._id !== "" &&
      imagePopupContent.card.owner !== ""
    )
    ...
    api
      ...
      .then((user) => {
        ...
        setInfoTooltipContent({
          ...
          message: intl.formatMessage({
            id: "profile_success",
            defaultMessage: "Профиль успешно обновлён!",
          }),
        });
      })
      .catch((err) => {
  ...
  return (
    <CurrentUserContext.Provider value={{ currentUser, setCurrentUser }}>
      <div className="page body__element">
        <Helmet>
          <html
            lang={intl.formatMessage({ id: "lang", defaultMessage: "ru" })}
          />
          <title>
            {intl.formatMessage({ id: "app_title", defaultMessage: "Место" })}
          </title>
          <noscript>
            {intl.formatMessage({
              id: "noscript",
              defaultMessage:
                "Вам нужно включить Javascript, чтобы запустить это приложение.",
            })}
          </noscript>
        </Helmet>
        ...
        <Switch>
            ...
            render={() => (
              <>
                ...
                <ImagePopup
                  ...
                  imagePopupContent={imagePopupContent}
                  onClose={closeAllPopups}
                />
                <ConfirmDeletePopup
          ...
          <Route
            path="/sign-up"
            render={() => <Register handleRegister={handleRegister} />}
          />
          <Route
            path="/sign-in"
